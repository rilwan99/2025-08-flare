...
contract(`CoreVaultManager.sol; ${getTestFile(__filename)}; CoreVaultManager unit tests`, (accounts) => {
    ...
    it("should not initialize contract if wrong parameters", async () => {
        ...
        await expectRevert.custom(tx, "InvalidAddress", []);
        ...
        await expectRevert.custom(tx, "InvalidChain", []);
    ...
    it("should not update settings if escrow end time is more than a day", async () => {
        ...
        await expectRevert.custom(tx, "InvalidEndTime", []);
    });
    ...
    it("should not update settings if fee is zero", async () => {
        ...
        await expectRevert.custom(tx, "FeeZero", []);
    });
    ...
    it("should revert adding preimage hashes if zero hash", async () => {
        ...
        await expectRevert.custom(tx, "InvalidPreimageHash", []);
    });
    ...
    it("should not pause the contract if not called by emergency pause senders", async () => {
        ...
        await expectRevert.custom(tx, "NotAuthorized", []);
    ...
    describe("request and cancel transfers", () => {
        ...
        it("should revert requesting multiple transfers from core vault - same destination address (cancelable)", async () => {
            ...
            await expectRevert.custom(
                coreVaultManager.requestTransferFromCoreVault(destinationAddress, paymentReference, 300, true, { from: assetManager }),
                "RequestExists", []
            );
        ...
        it("should revert requesting transfer if paused", async () => {
            await coreVaultManager.pause({ from: governance });
            await expectRevert.custom(
                coreVaultManager.requestTransferFromCoreVault("addr1", ZERO_BYTES32, 10, false, { from: assetManager }),
                "ContractPaused", []
            );
        });
        ...
        it("should revert requesting transfer if amount is zero", async () => {
            await expectRevert.custom(
                coreVaultManager.requestTransferFromCoreVault("addr1", ZERO_BYTES32, 0, false, { from: assetManager }),
                "AmountZero", []
            );
        });
        ...
        it("should revert requesting transfer if there are insufficient funds", async () => {
            ...
            await expectRevert.custom(
                coreVaultManager.requestTransferFromCoreVault("addr1", ZERO_BYTES32, 700, false, { from: assetManager }),
                "InsufficientFunds", []
            );
        });
        ...
        it("should revert canceling request transfer if request not found", async () => {
            await expectRevert.custom(
                coreVaultManager.cancelTransferRequestFromCoreVault("addr1", { from: assetManager }),
                "NotFound", []
            );
        });
        ...
        it("should not trigger instructions if payment fee is not set (is zero)", async () => {
            ...
            await expectRevert.custom(coreVaultManager.triggerInstructions({ from: accounts[1] }), "FeeZero", []);
        });
    });
    ...
    describe("trigger instructions", () => {
        ...
        it("should revert triggering instructions if not from triggering account", async () => {
            await expectRevert.custom(coreVaultManager.triggerInstructions({ from: accounts[2] }), "NotAuthorized", []);
        });

        it("should revert triggering instructions if paused", async () => {
            await coreVaultManager.pause({ from: governance });
            await expectRevert.custom(coreVaultManager.triggerInstructions({ from: accounts[1] }), "ContractPaused", []);
        });

        it("should not set escrow as finished if not from governance", async () => {
            await expectRevert.custom(coreVaultManager.setEscrowsFinished([preimageHash1], { from: accounts[1] }), "OnlyGovernance", []);
        });

        it("should revert setting escrow as finished if escrow not found", async () => {
            await expectRevert.custom(coreVaultManager.setEscrowsFinished([web3.utils.keccak256("wrong hash")], { from: governance }), "NotFound", []);
        });

        it("should revert setting escrow if already finished", async () => {
            await createEscrows();
            await coreVaultManager.setEscrowsFinished([preimageHash1], { from: governance });
            await expectRevert.custom(coreVaultManager.setEscrowsFinished([preimageHash1], { from: governance }), "EscrowAlreadyFinished", []);
        });
    ...
    describe("proxy upgrade", () => {
        ...
        it("calling initialize in upgradeToAndCall should revert in GovernedBase", async () => {
            ...
            await expectRevert.custom(
                coreVaultManagerProxy.upgradeToAndCall(newImpl.address, callData, { from: governance }),
                "GovernedAlreadyInitialized", []
            );
        });
        ...
        it("should revert if not upgrading from governance", async () => {
            ...
            // upgrade
            const newImpl = await CoreVaultManager.new();
            await expectRevert.custom(coreVaultManagerProxy.upgradeTo(newImpl.address), "OnlyGovernance", []);
            const callData = abiEncodeCall(coreVaultManager, (c) => c.pause());
            await expectRevert.custom(
    ...
    describe("ERC-165 interface identification", () => {
        it("should properly respond to supportsInterface", async () => {
            ...
            const IIAddressUpdatable = artifacts.require(
                "@flarenetwork/flare-periphery-contracts/flare/addressUpdater/interfaces/IIAddressUpdatable.sol:IIAddressUpdatable" as "IIAddressUpdatable"
            );
