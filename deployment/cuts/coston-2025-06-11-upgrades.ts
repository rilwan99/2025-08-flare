import { deployAgentVaultFactory, deployCollateralPoolFactory } from "../lib/deploy-asset-manager-dependencies";
...
runDeployScript(async ({ hre, artifacts, contracts, deployer }) => {
    ...
    // upgrade AgentVault implementation
    await deployAgentVaultFactory(hre, contracts);
    await assetManagerController.setAgentVaultFactory(assetManagers, contracts.getAddress("AgentVaultFactory"), { from: deployer });
    console.log(`AgentVaultFactory upgraded to ${await assetManagerFTestXRP.getSettings().then(s => s.agentVaultFactory)}`);
